...
export class FAQController {
  ...
  public async createQuestionAnswer(req: Request, res: Response) {
    ...
    if(!Array.isArray(body)) return CommonUtil.failResponse(res, 'payload should be an array');
    ...
    return CommonUtil.successResponse(res, 'Question and answer has been saved successfully');
  }
  ...
  public async editQuestionAnswer(req: Request, res: Response) {
    ...
    if(!isQuestionAnswerFound) return CommonUtil.failResponse(res, 'requested data is not found');
    if(question) await FAQ.updateOne({idx}, {question});
    if(answer) await FAQ.updateOne({idx}, {answer});

    return CommonUtil.successResponse(res, 'requested data has been edited successfully');
  }
  ...
  public async deleteQuestionAnswer(req: Request, res: Response) {
    ...
    return CommonUtil.successResponse(res, 'requested data has been deleted successfully');
  }
}
