...
export class MantaService implements multer.StorageEngine{
  ...
  private async mkdirInManta(MANTA_ROOT_FOLDER: string, PROJECT_NAME: string) {
    return new Promise((resolve, reject) => {
      (<any>this.manta).mkdirp(`~~/${MANTA_ROOT_FOLDER}/${PROJECT_NAME}`, (err: any, res: any) => {
  ...
  private async uploadFileToManta(destPath: string, fileStream: stream.Readable) {
  ...
  public async _handleFile(req: Request, file: Express.Multer.File, cb: (error: any, info?: Partial<Express.Multer.File>) => void) {
    try {
      ...
      if(file.fieldname === 'resource') {
        ...
        await this.mkdirInManta(<string>MANTA_ROOT_FOLDER, <string>PROJECT_NAME);
  ...
  public async _removeFile(req: Request, file: Express.Multer.File, cb: (error: any, info?: Partial<Express.Multer.File>) => void) {
    try {
      if (this.manta) {
        ...
        await new Promise((resolve, reject) => {
          ...
          (<manta.manta.MantaClient>this.manta).rmr(`~~/${PROJECT_NAME}/${file.originalname}`, (err: any, res: any) => {
