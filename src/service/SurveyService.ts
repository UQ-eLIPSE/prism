...
import {SurveyNode, MinimapConversion, MinimapNode, Survey, IMinimapNode} from "../models/SurveyModel";
...
export abstract class SurveyService {
  ...
  static async readZipFile(req: Request, res: Response, next: NextFunction) {
    ...
    if(file !== undefined) {
      ...
      zip.on('ready', async () => {
        ...
        const appFilesExist = entries.some((entry: any) => entry.name.endsWith('app-files/') && entry.isDirectory);
        const surveyJsonExist = entries.some((entry: any)=> entry.name.endsWith('survey.json'));
        const dataJsExist = entries.some((entry: any)=> entry.name.includes('app-files/data.js'));

        if(surveyJsonExist && appFilesExist && dataJsExist) {
          let surveyJson: any[] = [];

          SurveyService.readSurveyJson(entries, zip).then((data: any[])=>data.map(survey=> surveyJson.push(survey)));

          let surveyIds: any = await SurveyService.readFileData(user, entries, zip);
          ...
          if(surveyIdsArr.length !== surveyJson.length) {
            ...
            for(let idx = 0; idx < surveyIdsArr.length; idx++) {
              const surveysTable = await Survey.findOne({surveyNodes: surveyIdsArr[idx] as any});

              await SurveyNode.findByIdAndDelete(surveyIdsArr[idx]);
              await MinimapConversion.findOneAndDelete({surveyNode: surveyIdsArr[idx] as any});
              await MinimapNode.findOneAndDelete({surveyNode: surveyIdsArr[idx] as any});
            ...
            message = 'Number of scenes in survey.json does not match number of scenes from marzipano';
            ...
            for (let idx = 0; idx < surveyIdsArr.length; idx++) {
              const minimapConversion = await MinimapConversion.findOne({surveyNode: surveyIdsArr[idx] as any});
              const surveyNode = await SurveyNode.findById(surveyIdsArr[idx]);

              if(surveyNode) {
                await SurveyNode.findByIdAndUpdate(surveyIdsArr[idx], {date: surveyJson[idx]['date']});
              }

              if (minimapConversion) {
                const minimapNode = await MinimapNode.findOne({surveyNode: surveyIdsArr[idx] as any});
                await MinimapConversion.findOneAndUpdate({surveyNode: surveyIdsArr[idx] as any},{
                ...
                await MinimapNode.findOneAndUpdate({surveyNode: surveyIdsArr[idx] as any}, {
                  floor: surveyJson[idx]['floor']
                });
              }
            }
  ...
  static async readFileData(userDetails: any, entries: any, zip: any) {
    const dataJs = entries.filter((entry: any)=> entry.name.endsWith('/data.js'));
          ...
          stream.on('end', async ()=> {
            ...
            if(marzipanoData) {
              ...
              for(let idx = 0; idx < marzipanoData.scenes.length; idx++) {
                ...
                await Survey.findOneAndUpdate({_id: newSurvey._id}, {$push: {surveyNodes: newSurveyNode._id}});
  ...
  static async readSurveyJson(entries: any, zip: any) {
    const surveyJson = entries.filter((entry: any) => entry.name.endsWith('survey.json'));
    let minimapData: any;
  ...
  static async setSurveyPagination(maxResult: number, pageNo: number, size: number, res: Response, fieldToSearch?: object) {
    let mongoQuery: any = {};
